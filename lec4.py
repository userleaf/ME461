...
class ball:
    ...
    def join(ball1,ball2):
        ...
        '''
        global cp
        new_ball = ball(ball1.color,ball1.x,ball1.y,ball1.r+ball2.r,ball1.vx,ball2.vy,False)
        cp.append(new_ball)
...
def is_the_triangle_small(corners):
    '''
    ...
    for i in range(3):
        angle=getAngle(coords[i],coords[(i+1)%3],coords[(i+2)%3])
        distance=dista(coords[i],coords[(i+1)%3])
        if angle<=30 or distance<=2*r:
            return True
    return False
...
def dista(a,b): # this is for calculating the distance between two points
...
def triangle_bounce(ball):
    ...
    if ball.color==2:
        dx=(ball.x-mx)
        dy=(ball.y-my)
        dx,dy=dx/sqrt(dx**2+dy**2),dy/sqrt(dx**2+dy**2)
    elif ball.color==1:
        dx=(mx-ball.x)
        dy=(my-ball.y)
        dx,dy=dx/sqrt(dx**2+dy**2),dy/sqrt(dx**2+dy**2)
    ...
    ball.vx,ball.vy=v*dx,v*dy

def bigger_triangle():
    '''
    hocam we could not handle it
    '''
    pass

for i in range(randint(10,25)): # create some balls
    cp.append(ball(color=randint(0,2), x=randint(3*r,w-3*r),y=randint(3*r,h-3*r),r=r,vx=randint(1,4),vy=randint(1,4))) # create a ball
...
while True:
    ...
    if is_the_triangle_small(corners) and is_there_a_triangle: # if the triangle is small and there is a triangle
        try:
    ...
    for ii,i in enumerate(cp): # split and merge the balls
        ...
        for jj,j in enumerate(cp):
            if ii > jj and dist(i,j) < i.r+j.r : # if the balls are touching
                ...
                elif i.x>1+i.r and i.x<w-i.r+1 and i.y>1+i.r and i.y<h-i.r+1 and j.x>1+j.r and j.x<w-j.r+1 and j.y>1+j.r and j.y<h-j.r+1: # if the balls are touching    
        ...
        if i.corner==False:
            if ucgen.is_the_ball_in_the_triangle(i) and is_there_a_triangle: # if the ball is in the triangle
                if i.color==1 and is_it_touching_the_triangle(i) :  # if the ball is green and touches the triangle
                    triangle_bounce(i) # bounce the ball
                    i.move() # move the ball
            ...
            if ucgen.is_the_ball_in_the_triangle(i) and is_there_a_triangle:
                if i.color==0 and is_it_touching_the_triangle(i):
                    i.change_color(1)
    ...
    for i in cp:
        for j in cp:
            if i != j and i.isittouching==True and j.isittouching==True:
                unstuck_the_ball(i,j) # unstuck the balls
                i.isittouching=False
